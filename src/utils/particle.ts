...
export class Particle {
    ...
    constructor(
        ...
        let additionalDistance =
            this.particleStyle.radius + this.connectionsStyle.distance;
        this.boundingRox = new BoundingBox(
            boundingBox.x1 - additionalDistance,
            boundingBox.y1 - additionalDistance,
            boundingBox.x2 + additionalDistance,
            boundingBox.y2 + additionalDistance
        );
    ...
    randomize(spawnOnEdge: boolean = false) {
        this.constVelocity.setMagnitude(
            randomFromRange(this.velocityRange.min, this.velocityRange.max)
        );
                ...
                toHex(
                    Math.floor(
                        lerp(
                            ...
                            0,
                            Math.pow(
                                distance / this.connectionsStyle.distance,
                                2
                            )
                        )
                    ),
