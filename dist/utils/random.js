...
export function randomPoint(bounding_box) {
    return new Vector2d(randomFromRange(bounding_box.x1, bounding_box.x2 + 1), randomFromRange(bounding_box.y1, bounding_box.y2 + 1));
}
export function randomEdgePoint(bounding_box) {
    ...
    const edgeNum = (isHorizontalEdge ? 1 : 0) + (isHorizontalEdge != isLowEdge ? 2 : 0);
